...
import {
    ...
    Clock, Receipt, DollarSign, ArrowRight, X,
    Factory, FileText, AlertTriangle, ChevronDown
...
const statusStyle = (status) => {
    const map = {
        sent: 'bg-blue-100 text-blue-700 border-blue-200',         // SCM Sent it (New)
        production: 'bg-amber-100 text-amber-700 border-amber-200', // Supplier manufacturing
        shipping: 'bg-violet-100 text-violet-700 border-violet-200', // Out for delivery
        delivered: 'bg-emerald-100 text-emerald-700 border-emerald-200', // Reached warehouse
        completed: 'bg-slate-100 text-slate-700 border-slate-200'    // Paid & Received
    };
...
const statusText = (status) => {
    ...
    if (status === 'shipping') return 'For Delivery';
    if (status === 'delivered') return 'Delivered (Awaiting Receiving)';
...
const openInvoiceModal = (order) => {
    ...
    // Auto-generate a dummy invoice number for convenience
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="mb-6 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Purchase
                    Orders</h1>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-6">
            ...
            <div class="relative w-full sm:w-48">
                <select v-model="statusFilter"
                    ...
                    <option value="shipping">For Delivery</option>
        ...
        <div class="space-y-4">
            <div v-for="order in filteredOrders" :key="order.id"
                ...
                <div
                    class="flex flex-col md:flex-row justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                    <div>
                        <div class="flex items-center gap-2 mb-1.5">
                            <span class="font-mono text-sm font-black text-slate-800 dark:text-white">{{ order.po_number
                                }}</span>
                            <span
                                :class="['text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border', statusStyle(order.status)]">
                                {{ statusText(order.status) }}
                            </span>
                        </div>
                        <p class="text-xs text-slate-500 flex items-center gap-1.5">
                            <Clock class="w-3.5 h-3.5" /> Expected Delivery: <strong
                                class="text-slate-700 dark:text-slate-300">{{ order.expected_delivery }}</strong>
                        </p>
                    </div>
                    <div class="md:text-right">
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Order Total</p>
                        <p class="text-xl font-black text-emerald-600 dark:text-emerald-400 leading-none mt-0.5">{{
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Billing & Payments
                            </p>
                        ...
                        <div v-else class="space-y-2">
                            <div v-for="inv in order.invoices" :key="inv.id"
                                ...
                                <div class="flex justify-between items-center mb-1.5">
                                    ...
                                    <span
                                        :class="['text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md', inv.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700']">
                                        {{ inv.status }}
                                    </span>
                                </div>
                ...
                <div class="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2 justify-end">
                    ...
                    <button v-if="order.status === 'production'" @click="updateStatus(order.id, 'shipping')"
                        ...
                        <Truck class="w-4 h-4" /> Ship Order / For Delivery
                    </button>

                    <button v-if="order.status === 'shipping'" @click="updateStatus(order.id, 'delivered')"
                        :disabled="processingId === order.id"
                        class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-black shadow-sm transition-all active:scale-[0.98] disabled:opacity-50">
                        <CheckCircle class="w-4 h-4" /> Mark as Delivered
                    </button>

                    <div v-if="['delivered', 'completed'].includes(order.status)"
                        ...
                        <Package class="w-4 h-4" /> Awaiting Monti Warehouse Receiving
                    </div>
                </div>
            </div>
        </div>
        ...
        <Teleport to="body">
            <div v-if="showInvoiceModal"
                ...
                <div
                    class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md overflow-hidden flex flex-col">
                    <div
                        class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex justify-between items-center">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Invoice
                                Number *</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Due Date
                                    *</label>
                        ...
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Amount
                                to Bill (₱) *</label>
                    ...
                    <div
                        class="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex gap-3 bg-slate-50 dark:bg-slate-800">
                        <button @click="showInvoiceModal = false"
